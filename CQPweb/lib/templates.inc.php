...
function list_xml_templates()
{
	...
	$result = do_mysql_query("select * from xml_template_info order by id");
	...
	$result = do_mysql_query("select * from xml_template_content order by template_id, order_in_template");
 ...
 * @param $attributes   An array of s-attribute specifications. This must contain either inner-arrays or objects, each of which is a
...
{
	$description  = mysql_real_escape_string($description);
	do_mysql_query("insert into xml_template_info (description) values ('$description')");
	$id = get_mysql_insert_id(); 
	...
	for ($i = 1 ; isset($attributes[$i]) ; $i++)
	{
		...
		$sql .= ' \'' . cqpweb_handle_enforce($a->att_family) . '\', ' ;
...
{
	$id = (int) $id;
	do_mysql_query("delete from xml_template_info where id = $id");
	do_mysql_query("delete from xml_template_content where template_id = $id");
}
...
function list_metadata_templates()
{
	...
	$result = do_mysql_query("select * from metadata_template_info order by id");
	...
	$result = do_mysql_query("select * from metadata_template_content order by template_id, order_in_template");
...
{
	$description  = mysql_real_escape_string($description);
	do_mysql_query("insert into metadata_template_info (description) values ('$description')");
	$id = get_mysql_insert_id(); 
	...
	for ($i = 1 ; isset($fields[$i]) ; $i++)
	{
		...
		$sql .= ' \'' . mysql_real_escape_string($f->description) . '\', ';
	...
	if (!empty($primary_classification))
		if (in_array($primary_classification, $collect_handles))
			do_mysql_query("update metadata_template_info set primary_classification = '$primary_classification' where id = $id");
}

function delete_metadata_template($id)
{
	$id = (int) $id;
	do_mysql_query("delete from metadata_template_info where id = $id");
	do_mysql_query("delete from metadata_template_content where template_id = $id");
}
